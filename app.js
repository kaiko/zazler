  ...
  templNewDb() {
    ...
    return Object.create({}, {
      ...
      query: { value: async (tf, vars = {}) => {
        let aSch;
        if (!a._schema) aSch = await a.schema();
        if (!a._schema) throw new Exception(500, new Error());
        let [table, fmt] = breakOn(tf, '.');
        let [t, as] = breakOn(table, '@');
        let qRes = await a.runSelect(t, as, vars, this.meta, aSch, this.filt, {});
        return !fmt ? qRes : (await a.format(qRes, fmt, table, as, vars, { isMain: false, isPost: false, vars: vars, url: tf}, meta, aSch )).text();
      } }
    })
  }
